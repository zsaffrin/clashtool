...
class userModel {
	...
	public function setPassword() {
		...
		if (empty($_POST['new_password'])) {
			...
			$sql = 'UPDATE 	users
					...
					WHERE 	user_id = :userid';
			$query = $this->db->prepare($sql);
			$query->execute(array(':code' => $newCode, ':hpass' => $hpass, ':userid' => $_POST['userid']));
	...
	public function changePassword() {
		...
		if (empty($_POST['password'])) { 
			...
			// Hash new password
			$newCode = sha1(time());
			$newPass = hash_hmac('sha512', $_POST['new_password'], $newCode);
			...
			$sql = 'UPDATE 	users
					...
					WHERE 	user_id = :userid';
			$query = $this->db->prepare($sql);
			$query->execute(array(':code' => $newCode, ':newpass' => $newPass, ':userid' => $user->user_id));
